...
/*
  - 방문자 기록 기능
    * 서버 측에 방문자 기록을 DB에 기록함
    * 동일 IP가 방문한 경우, 시간 차를 두도록 할 것

  - 메모 기능
    * 맨 밑에 텍스트 필드와 전송 버튼, 지우기 버튼이 있음
    * 로그인 상태일 경우: 페이지를 로드할 때 메모 내용을 로드해서 렌더링함, 전송 버튼을 누르면 텍스트 내용을 DB에 전송, 지우기 버튼을 누르면 텍스트 필드 및 DB의 내용 삭제
  - 읽기 체크 기능
    * 절 버튼 옆에 체크 버튼으로 읽었는지 여부를 체크
  - 개인 정보 관리 페이지
    * 백업: 메모, 읽기 체크, 책갈피 정보를 백업해서 파일로 다운로드
    * 복구: 백업한 파일을 보내주면 개인 정보 DB에 반영함
    * 초기화: 읽기 체크, 메모, 책갈피 중 선택한 항목에 대한 데이터를 초기화
*/
...
async function addBookmark() {
    ...
    if (localStorage.getItem('token')) {
        ...
        let newEntry = [];
        newEntry.push(current_book_name);
        newEntry.push(current_chapter_number);
        newEntry.push(tag_name);
        bookmarkInfo.push(newEntry);
        localStorage.setItem('bookmark', JSON.stringify(bookmarkInfo));
    }
